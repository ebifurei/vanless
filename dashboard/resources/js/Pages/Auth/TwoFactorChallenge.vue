    ...
    <SectionFullScreen v-slot="{ cardClass }" bg="purplePink">
      <CardBox :class="cardClass" is-form @submit.prevent="submit">
        ...
        <FormField v-if="!recovery" label="Code" label-for="code" help="Please enter one-time code">
          <FormControl id="code" @set-ref="codeInput = $event" v-model="form.code" type="text" inputmode="numeric"
            autofocus autocomplete="one-time-code" />
        </FormField>

        <FormField v-else label="Recovery Code" label-for="recovery_code" help="Please enter recovery code">
          <FormControl id="recovery_code" @set-ref="recoveryCodeInput = $event" v-model="form.recovery_code" type="text"
        ...
        <BaseLevel>
          <BaseButton type="submit" color="info" label="Log in" :class="{ 'opacity-25': form.processing }"
